<!-- <template>
  ...
  <audio id="song"  class="hidden" ref="audioRef"></audio>
...
import { SpectrumAnalyzer } from './SpectrumAnalyzer'; // Assuming you have SpectrumAnalyzer in a separate file
...
function initAudio() {
  ...
  // mAudioElement.src = 'https://audio.jukehost.co.uk/Au9w15zADfVg22H81YzswFq7HgzaHvix';
	mAudioElement.src = 'https://audio.jukehost.co.uk/7jpLnuHZc0a7Q7rST4op74HyVcNxvcuD';
...
function initTHREE() {
  ...
  mLight2 = new THREE.DirectionalLight(0xFFFFFF, 0.25);
  mLight2.position.set(0, 1, 1);
  mScene.add(mLight2);

  mLight3 = new THREE.DirectionalLight(0xFFFFFF, 0.25);
...
function initParticleSystem() {
  // var prefabGeometry = new THREE.PlaneGeometry(4, 4);
	var prefabGeometry = new THREE.SphereGeometry(2, 4, 4);
  ...
  var prefabDelay = 0.0069;
  // var vertexDelay = 0.0175;
	var vertexDelay = 0.069;
  // var minDuration = 32.0;
	var minDuration = 16.0;
...
//     }
//   }
	var color = new THREE.Color(0x111111);
	for (i = 0, offset = 0; i < mParticleCount; i++) {
			for (j = 0; j < prefabGeometry.vertices.length; j++) {
					aColor.array[offset++] = color.r;
					aColor.array[offset++] = color.g;
